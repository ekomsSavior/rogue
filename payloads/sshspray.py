    ...
    def worker(self):
        ...
        while True:
            try:
                ...
                with self.lock:
                    already = target in self.success_targets
                if already:
                ...
                if self.try_ssh_login(target, username, password):
                    with self.lock:
                        self.success_targets.add(target)
...
def rogue_integration():
    ...
    # Load credentials (files first, then merge inline lists - never discard)
    ...
    if args.usernames:
        sprayer.usernames.extend([u.strip() for u in args.usernames.split(',') if u.strip()])
    if args.passwords:
        sprayer.passwords.extend([p.strip() for p in args.passwords.split(',') if p.strip()])
    # dedupe, preserve order
    sprayer.usernames = list(dict.fromkeys(sprayer.usernames))
    sprayer.passwords = list(dict.fromkeys(sprayer.passwords))
