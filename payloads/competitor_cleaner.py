        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'username']):
            try:
                ...
                for pattern in self.suspicious_patterns['process_names']:
                    if re.search(pattern, pinfo.get('name') or '', re.I) or re.search(pattern, cmdline, re.I):
                        suspicious.append({
                            'pid': pinfo['pid'],
                            'name': pinfo['name'],
                            'cmdline': cmdline[:200],
        ...
        for cron in crons:
            try:
                if cron['source'] == 'user_crontab':
                    ...
                    # Filter suspicious lines out of the file instead of
                    # deleting the whole file (keeps legit scheduled jobs)
                    if os.path.exists(cron['source']):
                        try:
                            with open(cron['source'], 'r') as f:
                                lines = f.readlines()
                            pats = self.suspicious_patterns['cron_patterns']
                            kept = [ln for ln in lines
                                    if not any(re.search(p, ln, re.I) for p in pats)]
                            if len(kept) != len(lines):
                                with open(cron['source'], 'w') as f:
                                    f.writelines(kept)
                                removed.append({
                                    'type': 'cron_file_filtered',
                                    'path': cron['source'],
                                    'reason': cron['reason']
                                })
                                print(f"[+] Filtered cron file: {cron['source']}")
                            elif len(kept) == 0:
                                ...
                                print(f"[+] Removed empty cron file: {cron['source']}")
                        except Exception as e:
                            print(f"[!] Failed to filter cron file: {e}")
            except Exception as e:
