...
def enumerate_s3_buckets(credentials):
    """Enumerate S3 buckets using discovered credentials (real API calls)"""
    buckets = []
    try:
        import boto3
        import botocore.session as _bcs
    except ImportError:
        buckets.append("s3 enumeration requires boto3 on target")
        return buckets

    try:
        kwargs = {
            "aws_access_key_id": credentials.get("access_key_id")
            or credentials.get("secret_access_key_id"),
            "aws_secret_access_key": credentials.get("secret_access_key"),
            "aws_session_token": credentials.get("token"),
        }
        if not kwargs["aws_access_key_id"] or not kwargs["aws_secret_access_key"]:
            buckets.append("no usable AWS credentials for S3 enumeration")
            return buckets
        client = boto3.client("s3", **{k: v for k, v in kwargs.items() if v})
        for b in client.list_buckets().get("Buckets", []):
            buckets.append(b["Name"])
    except Exception as e:
        buckets.append(f"s3 enumeration error: {str(e)}")
    return buckets
